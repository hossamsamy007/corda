...
//        ClassNotOnClasspath(BOB_NAME, 54321).serialize().open().copyTo("build" / "class-not-on-classpath-data")
    ...
    @Test
    fun `PartyAndCertificate serialisation with isFullParty = true`() {
        partyObjectMapper.isFullParties = true
    ...
    @Test
    fun `PartyAndCertificate deserialisation on cert path`() {
    ...
    @Test
    fun `X509Certificate serialisation`() {
    ...
    @Test
    fun `X509Certificate deserialisation`() {
        val cert: X509Certificate = MINI_CORP.identity.certificate
